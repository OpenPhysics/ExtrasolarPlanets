...
main.ts
  └─ ExtrasolarPlanetsScreen            (Screen<ExtrasolarPlanetsModel, ExtrasolarPlanetsScreenView>)
       ├─ ExtrasolarPlanetsModel         owns all state
       └─ ExtrasolarPlanetsScreenView    owns all visuals
```
...
Mirror the structure of `src/extrasolar-planets-screen/`:
...
existing `ExtrasolarPlanetsScreen.ts`.
...
const icon = new ScreenIcon(
  new Rectangle(0, 0, 548, 373, { fill: ExtrasolarPlanetsColors.accentColorProperty }),
