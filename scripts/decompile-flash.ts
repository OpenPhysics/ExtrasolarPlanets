 ...
 * Flash sources under `NAAP/`, so the ported physics can be checked against the
 ...
 *
 * Run with:
 *   npm run decompile                 # the two ESP simulators → NAAP/decompiled/
 *   npm run decompile -- --all        # all ESP-relevant movies
...
const ESP_TARGETS = [
  // The two simulators being ported (primary source of truth).
  "radialVelocitySimulator/radialVelocitySimulator012.swf",
  "transitSimulator/transitSimulator017.swf",
  // Supporting RV/transit components and demos.
  "radialVelocityComponent/radialVelocityComponent010.swf",
  "radialVelocityDemo/radialVelocityDemo003.swf",
  "transitMovie/transitMovie041.swf",
  "exoplanetOrbitDiagram/exoplanetOrbitDiagram007.swf",
  "exoplanetComboDiagram/exoplanetComboDiagram004.swf",
  "centerOfMass/centerOfMass009.swf",
  "lightcurveComponentII/lightcurveComponentII006.swf",
  // Shared physics helpers (Gaussian noise, spectral type↔temp, blackbody, formatting).
  "gaussianRandomNumber/gaussianRandomNumber001.swf",
  "typeAndTemp/typeAndTemp014.swf",
  "simpleBlackbody/simpleBlackbody017.swf",
  "numberFunctions/numberFunctions002.swf",
].map((p) => join(flashRoot, p));

/** The two simulators decompiled by default when no targets are given. */
const DEFAULT_TARGETS = ESP_TARGETS.slice(0, 2);
...
Usage:
  npm run decompile                  Decompile the two ESP simulators → NAAP/decompiled/
  npm run decompile -- --all         Decompile all ESP-relevant movies
  npm run decompile -- <path>…       Decompile specific .swf / .fla / folder(s)

Flags:
  --all              Decompile the curated set of ESP-relevant SWFs
